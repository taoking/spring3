...
class GlobalExceptionHandler {
    ...
    ProblemDetail handleProductNotFound(ProductNotFoundException ex, HttpServletRequest request) {
        ProblemDetail detail = ProblemDetail.forStatusAndDetail(HttpStatus.NOT_FOUND, ex.getMessage());
        detail.setTitle("Product not found");
        detail.setType(URI.create("https://spring3.local/problems/product-not-found"));
    ...
    ProblemDetail handleSimulatedFailure(SimulatedCatalogException ex, HttpServletRequest request) {
        Sentry.captureException(ex);
        ProblemDetail detail = ProblemDetail.forStatusAndDetail(HttpStatus.INTERNAL_SERVER_ERROR, ex.getMessage());
        detail.setTitle("Simulated catalog failure");
        detail.setType(URI.create("https://spring3.local/problems/catalog-simulated-failure"));
        detail.setProperty("path", request.getRequestURI());
        return detail;
    }
