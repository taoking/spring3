...
| `catalog-service` native binary 编译 | 已尝试，失败原因是本机缺少 `native-image` |
...
这说明当前代码至少可以完成 Spring AOT 处理；如果要继续生成本地二进制，需要先安装 GraalVM JDK 21 并确保 `native-image` 在 `PATH` 中，或改用 Docker buildpacks 构建 native image。
...
| 自定义 starter / AOP | `@DemoLog` 依赖 Spring AOP 代理 | 如果 native 编译报反射或代理错误，再补 `RuntimeHints` |
